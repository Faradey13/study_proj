...
import {useHidding} from "../components/hooks/useHidding";
...
const PostsPage: FC = () => {
    ...
    const[modal, setModal] = useHidding('Escape')

    const pagesArray = usePaginations(totalPage)

    useObserve(isLoading, page < totalPage, lastElement, () => {
        setPage(page + 1)
    })
    ...
    return (
        <div>
            ...
            <MySelect defaultValue={"Показывать по:"} value={limit}
                      onChange={event => setLimit(Number(event.target.value))} options={[
                {value: 5, name: '5'},
                {value: 10, name: '10'},
                {value: 25, name: '25'},
